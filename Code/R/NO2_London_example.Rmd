...
gg = ggplot(data=post_samples, aes(x=.iteration, y = alpha3, color=.chain))+
  geom_line() + theme_minimal(base_line_size = 14)
gg
```
...
df_site_test = df_site_test[order(df_site_test$SiteCode),]
...
```{r}
file = paste0(path1,'/GP_3d_kron_alltwoway_pred.stan')
...
for (s in site_list_test){
  ...
  df_day_bysite$date = as.POSIXct( (df_NO2$date),tz='GMT')
  gg = ggplot() +
